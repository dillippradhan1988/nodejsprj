...
module.exports = {
	...
	home: function(req, res, next) {
		Customer.findById(req.params.id, function(err, customer) {
			...
			customer.getOrders(function(err, orders) {
				if(err) return next(err);
				res.render('customer/home', customerViewModel.getCustomerViewModel(customer, orders));
			});
		});
	},
	...
	preferences: function(req, res, next) {
		Customer.findById(req.params.id, function(err, customer) {
			...
			customer.getOrders(function(err, orders) {
				if(err) return next(err);
				res.render('customer/preferences', customerViewModel.getCustomerViewModel(customer, orders));
			});
		});
	},
